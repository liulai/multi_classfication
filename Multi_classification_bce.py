...
import copy
import numpy as np
from Multi_NetWork import Net, Net_BCE
import datetime
...
class MyDataSet(Dataset):
    def __init__(self, root_dir, anno_file, transform=False):
        ...
        self.file_info = self.file_info.astype('str')
        self.file_info = pd.concat([self.file_info, pd.get_dummies(self.file_info[['classes', 'species']])], axis=1)
    #
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'image': image, 'classes': int(label_classes), 'species': int(label_species)}
        sample_x = []
        for x in self.file_info.columns:
            if x not in ['path', 'classes', 'species']:
                sample_x.append(self.file_info[x][idx])
        sample['target'] = torch.tensor(sample_x, dtype=torch.float)
        if (self.transform):
...
def visual_dataset():
    ...
    print(idx, sample['image'].shape, CLASSES[int(sample['classes'])], SPECIES[int(sample['species'])])
...
def train_model(net, optimizer, criterion, scheduler, epochs=50):
    Loss_list = {'train': [], 'val': []}
    ...
    for epoch in range(epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for idx, data in enumerate(data_loader[phase]):
                ...
                labels_target = []
                labels_target = torch.tensor([np.array(i) for i in data['target']]).to(device)
                optimizer.zero_grad()
                #
                with torch.set_grad_enabled(phase == 'train'):
                    outputs = net(inputs)
                    _, pre_classes = torch.max(outputs[:, :2], 1)
                    _, pre_species = torch.max(outputs[:, 2:], 1)
    ...
    torch.save(net.state_dict(), './model_pt/best_model_bce_%s.pt'%time_str)
    print('Final val Classes Acc: {:.4f}'.format(best_acc))
    return net, Loss_list, Accuracy_list


net = Net_BCE().to(device)
...
net,Loss_list,Accuracy_list=train_model(net, optimizer, criterion, epoch_lr_scheduler, epochs=100)
# optim.Adam()
...
time_str=datetime.datetime.now().strftime('%Y%m%d%H%M%S')
anno_loss_list.to_csv('./csv/loss_list_bce_%s.csv' % time_str)
...
anno_acc_list=pd.DataFrame(Accuracy_list)
time_str=datetime.datetime.now().strftime('%Y%m%d%H%M%S')
anno_acc_list.to_csv('./csv/acc_list_bce_%s.csv' % time_str)
